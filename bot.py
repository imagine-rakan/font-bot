...
def main():
    ...
    # استخدام group=1 يجعله يعمل بشكل مستقل تماماً عن رسائل البوت العادية
    app.add_handler(MessageHandler(filters.TEXT | filters.CAPTION, delete_admin_chars), group=1)
    
    logging.info("--> Bot started")
    app.run_polling()
    
    app.add_handler(MessageHandler(filters.Regex(r"^(رفع مشرف بوت|/promote)$"), promote_user))
    app.add_handler(MessageHandler(filters.Regex(r"^(تنزيل مشرف بوت|إزالة مشرف بوت|/demote)$"), demote_user))

    # === أوامر الهمسة الجديدة ===
    # وضعناها في group=2 لكي تعمل بشكل مستقل تماماً عن الردود التلقائية وعن فلاتر المشرفين
    app.add_handler(MessageHandler(filters.Regex(r"^همسة "), handle_whisper), group=2)
    
    # وضعنا pattern محدد (wsp|) لكي لا يتعارض أبداً مع أزرار لوحة تحكم المشرفين
